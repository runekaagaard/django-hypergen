...
Python functions all the way down. Notice the ``cb()`` callback function automatically connecting the frontend onclick event to the backend that then re-renders the page with the counter increased by one. Se the `documentation <https://hypergen.it/documentation/>`_ for full example apps.
